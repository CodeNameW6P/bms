...
import { fetchFlatContributionsApi } from "@/api/contributionApi";

const ViewContributionsPage: React.FC = () => {
	...
	const fetchFlatContributions = async () => {
		const response = await fetchFlatContributionsApi(
			flat._id,
			...
			<Card className="flex grow container mx-auto">
				<CardHeader>
					<CardTitle className="text-2xl">
						{flat && flat.flatNumber} Flat Mosque Contributions
					</CardTitle>
				</CardHeader>
					...
					<Card className="">
						<CardHeader className="">
							...
							<CardAction className="flex gap-4">
								...
								<Button
									onClick={() => fetchFlatContributions()}
									variant={"outline"}
								>
									Refresh
								</Button>
								...
								<TableBody>
									{contributions.map((contribution: any, index) => (
										<TableRow key={index}>
											...
											<TableCell>{contribution.flat.flatNumber}</TableCell>
											<TableCell>{MONTHS[contribution.month - 1]}</TableCell>
